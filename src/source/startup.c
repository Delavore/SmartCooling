...
extern const uint8_t __ram_func_source_start;
extern uint8_t __ram_func_target_start;
extern uint8_t __ram_func_target_end;
...
extern void trap_handler_raw(void) __attribute__((interrupt("machine"),
                                    section(".text.riscv_trap_handler_raw")));
...
volatile void trap_handler_raw(){
    trap_handler();
}
...
void _start(void) {
    ...
    memcpy((void*)&__data_target_start, (const void*)&__data_source_start, (&__data_target_end - &__data_target_start));
     ...
     // Copy .ram_func section from ROM to RAM
    memcpy((void*)&__ram_func_target_start, (const void*)&__ram_func_source_start, (&__ram_func_target_end - &__ram_func_target_start));
